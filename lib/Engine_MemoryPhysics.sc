...
Engine_MemoryPhysics : CroneEngine {
	...
	*new { arg context, doneCallback;
	...
	alloc {
		...
		SynthDef(\FX_Bypass, { arg in, out; Out.ar(out, In.ar(in, 2)); }).add;

		SynthDef(\FX_Abyss, { arg in, out, p1=0.5, p2=0.5, p3=0.5, bpmBus;
			var sig = In.ar(in, 2);
			var monoDry = (sig.sum * 0.5) + WhiteNoise.ar(1e-8); 
			var sp1_fast = Lag.kr(p1, 0.05); var sp1_slow = Lag.kr(p1, 0.8);  
			var sp2 = Lag.kr(p2, 0.05); var sp3 = Lag.kr(p3, 0.05);
			...
			shimmerLoop = LeakDC.ar(shimmerLoop); shimmerLoop = shimmerLoop.tanh;
			LocalOut.ar(shimmerLoop * (sp2 * 0.85));
			wetAbyss = (monoDry + (shimmerLoop * sp2)) * 0.4;
			8.do { var badFirewall;
				wetAbyss = AllpassC.ar(wetAbyss, 0.1, LFNoise2.kr(0.1 + (sp3 * 0.5)).range(0.01, 0.05 + (sp3 * 0.04)), 1.0 + (sp1_slow * 5.0));
			...
			wetAbyss = wetAbyss.tanh; wetAbyss = Limiter.ar(wetAbyss * 1.4, 0.95, 0.01);
		...
		SynthDef(\FX_Breeze, { arg in, out, p1=0.5, p2=0.5, p3=0.5, bpmBus;
			var sig = In.ar(in, 2);
			var sp1 = Lag.kr(p1, 0.05); var sp2 = Lag.kr(p2, 0.05); var sp3 = Lag.kr(p3, 0.05);
		...
		SynthDef(\FX_Shuffler, { arg in, out, p1=0.5, p2=0.5, p3=0.5;
			var sig = In.ar(in, 2);
			var trig = Impulse.ar(10 * p1);
			var shuffler = DelayC.ar(sig, 2.0, LFNoise1.kr(p1 * 5).range(0.01, 1.0));
			var freeze = Latch.ar(sig, p2 > 0.8);
			var wet = SelectX.ar(p2, [shuffler, freeze]);
			Out.ar(out, XFade2.ar(sig, wet, (p3 * 2) - 1));
		}).add;
		...
		this.addCommand(\select_fx, "i", { arg msg;
			var fx_type = msg[1];
			var defs = [\FX_Bypass, \FX_Abyss, \FX_Harmony, \FX_Breeze, \FX_Crackle, \FX_Shuffler];
			fxSynth.free;
	...
	free { oscFuncs.do(_.free); fxBus.free; eqBus.free; fxSynth.free; eqSynth.free; }
}
